...
namespace Odyssey.Grains.Sessions
{
    public class DeviceGrain : Grain, IDeviceGrain
    {
        ...
        private readonly IAsyncStream<SignalROutboundEvent> _deviceEventStream;

        public DeviceGrain(
            [PersistentState(nameof(DeviceGrainState), GrainConstants.GrainStorage)] IPersistentState<DeviceGrainState> state,
            IGrainFactory<string, IJoinCodeGrain> joinCodeGrainFactory,
            IGrainFactory<Guid, IDeviceGrain> grainFactory,
            ...
            )
        {
            ...
            _logger = logger;
            _deviceEventStream = this.GetStreamProvider(GrainConstants.SignalRStreams)
                .GetStream<SignalROutboundEvent>(GrainConstants.DeviceEventsStreamNamespace, _id);
        }

        public Task<PartyMemberProfile> GetMemberProfileAsync() => Task.FromResult<PartyMemberProfile>(_state.State.Profile);

        public Task<DeviceProfile> GetProfileAsync() => Task.FromResult(_state.State.Profile);
        ...
        public async Task<DetailedResult<IMemberPartyGrain, JoinPartyReason>> JoinPartyAsync(string joinCode)
        {
            ...
            try
            {
                ...
                if (await party.JoinAsync(this.AsReference<IDeviceGrain>(), joinCode) != true)
                    return new(JoinPartyReason.PartyDoesNotExist);
        ...
        public async Task<DetailedResult<LeavePartyReason>> LeavePartyAsync(string joinCode)
        {
            ...
            if (await currentParty.LeaveAsync(this.AsReference<IDeviceGrain>(), joinCode) == false)
                return new(LeavePartyReason.PartyDoesNotExist);
            ...
            await _deviceEventStream.OnNextAsync(new DeviceSelfLeftPartyOutboundEvent());
        ...
        public async Task NotifyPartyDisbandedAsync(string joinCode)
        {
            ...
            {
                await _deviceEventStream.OnNextAsync(new PartyDisbandedOutboundEvent { PartyId = joinCode });
            }
        }

        public async Task SetProfileAsync(DeviceProfile profile)
        {
        ...
        public async Task<Optional<MemberPartyDetails>> GetMemberPartyAsync()
        {
            ...
            try
            {
                return await party.GetPartyDetailsAsync(this.AsReference<IDeviceGrain>(), _state.State.Profile);
            }
        ...
        public Task NotifyPartyMemberJoinedAsync()
        {
            ...
            return _deviceEventStream.OnNextAsync(new PartyMemberJoinedOutboundEvent());
        }
        ...
        public Task NotifyPartyMemberLeftAsync()
        {
            ...
            return _deviceEventStream.OnNextAsync(new PartyMemberLeftOutboundEvent());
        }
    }
}
