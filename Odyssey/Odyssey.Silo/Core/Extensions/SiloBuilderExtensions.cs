...
namespace Odyssey.Silo.Core.Extensions
{
    public static class SiloBuilderExtensions
    {
        extension(ISiloBuilder builder)
        {
            ...
            public ISiloBuilder ConfigureCluster(IConfiguration configuration)
            {
            ...
            public ISiloBuilder AddGrainStorage(IConfiguration configuration, string storageName)
            {
                var persistenceSettings = configuration.GetSection<PersistenceSettings>();
                switch (persistenceSettings.DatabaseSettings.Driver)
                {
                    ...
                    case DatabaseDriver.Memory:
                        builder.AddMemoryGrainStorage(storageName);
                        break;
                    case DatabaseDriver.Postgres:
                        break;
                    default:
